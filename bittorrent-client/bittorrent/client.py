...
class TorrentClient():
    ...
    def __init__(self, torrent_file, download_destination, loop, port, ip, download_times=1):
        ...
        #self.peers = [Peer(p['hostname'], p['port'], self.torrent)
                #for p in peers_list]
        self.peers = [Peer(port, ip, self.torrent)]
        print(self.peers)
        self.active_peers = []
        self.file_manager = FileManager(self.torrent, download_destination)
        self.downloaded = 0 # how many times you have downloaded?
        self.download_required = download_times
    ...
    async def _message_handler(self, peer, msg_id, payload):
        ...
        if msg_id == 0:
            ...
            for index in pieces_indexes:
                peer.queue.add(index)
            self.available_buffers = copy.deepcopy(pieces_indexes)

            self.logger.info('{} has {}'.format(peer.address['host'], pieces_indexes))
            if no_pieces:
                await self._request_piece(peer)

        elif msg_id == 6: # not implemented???
